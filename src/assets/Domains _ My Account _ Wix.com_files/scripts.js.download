...
(function() {
    ...
    var Ctrl = [ "$scope", "$element", "$transclude", function($scope, $element, $transclude) {
        if (!$transclude) {
            throw new Error("Illegal use of ngMultiTransclude controller. No directive " + "that requires a transclusion found.");
        }
...
angular.module("wixStyle", [ "wixStyleInternal", "styleTranslations", "rzModule", "multi-transclude" ]).config(function() {
    return;
});
            ...
            if (self.modalContainer === null) {
                ...
                if (options.clickOutsideToClose) {
                    self.modalContainer.on("click", function(e) {
                        ...
                        if (element.hasClass("wix-modal--container") || element.hasClass("wix-modal-window-wrapper")) {
                            self.close("BLUR");
                        }
                    });
                }
...
(function(directives) {
    var WixPageController = function() {
        function WixPageController() {
            this.handlers = {
                headerHeight: [],
                scrollTop: []
            };
        }
        WixPageController.prototype.registerHandler = function(key, handler) {
            this.handlers[key].push(handler);
        };
        WixPageController.prototype.triggerHandler = function(key, value) {
            this.handlers[key].forEach(function(handler) {
                handler(value);
            });
        };
        return WixPageController;
    }();
    directives.WixPageController = WixPageController;
    directives.wixPageDirective = function() {
        return {
            controller: WixPageController,
            template: '<div class="wix-page" ng-transclude></div>',
            transclude: true
        };
    };
})(directives || (directives = {}));

angular.module("wixStyle").directive("wixPage", directives.wixPageDirective);

var directives;

(function(directives) {
    var WixPageHeaderController = function() {
        WixPageHeaderController.$inject = [ "$scope" ];
        function WixPageHeaderController($scope) {
            this.$scope = $scope;
            this.contentStyle = {};
        }
        WixPageHeaderController.prototype.hasTitle = function() {
            return this.title;
        };
        WixPageHeaderController.prototype.hasPageTail = function() {
            return this.$scope.height !== this.$scope.coverTailHeight;
        };
        WixPageHeaderController.prototype.hasBreadcrumbs = function() {
            return !!this.breadcrumbs && this.breadcrumbs.length > 0;
        };
        WixPageHeaderController.prototype.hasBackgroundImage = function() {
            return !!this.backgroundImageUrl;
        };
        WixPageHeaderController.prototype.hasGradientClassName = function() {
            return !!this.gradientClassName;
        };
        WixPageHeaderController.prototype.backButtonClickHandler = function() {
            if (typeof this.onBackClicked === "function") {
                this.onBackClicked();
            }
        };
        WixPageHeaderController.prototype.isThemeLight = function() {
            return !this.isMinimized() && this.hasBackgroundImage();
        };
        WixPageHeaderController.prototype.isMinimized = function() {
            return this.$scope.minimized;
        };
        WixPageHeaderController.prototype.getContentStyle = function() {
            this.contentStyle.maxWidth = this.maxWidth + "px";
            this.contentStyle.paddingLeft = this.contentStyle.paddingRight = this.sidePadding + "px";
            return this.contentStyle;
        };
        return WixPageHeaderController;
    }();
    directives.WixPageHeaderController = WixPageHeaderController;
    directives.wixPageHeaderDirective = function() {
        return {
            controller: WixPageHeaderController,
            bindToController: true,
            controllerAs: "$ctrl",
            restrict: "E",
            require: "^wixPage",
            link: function(scope, element, attrs, wixPageController) {
                scope.minimized = false;
                setTimeout(function() {
                    var height = getHeaderHeight();
                    wixPageController.triggerHandler("headerHeight", height);
                    scope.coverTailHeight = height;
                    var pageTail = element[0].querySelector('[data-hook="wix-page-header-page-tail"]');
                    height -= pageTail.offsetHeight;
                    scope.height = height;
                });
                wixPageController.registerHandler("scrollTop", function(scrollTop) {
                    var shouldMinimize = scrollTop > 20;
                    if (scope.minimized !== shouldMinimize) {
                        scope.minimized = shouldMinimize;
                        setTimeout(function() {
                            var height = getHeaderHeight();
                            wixPageController.triggerHandler("headerHeight", height);
                        }, 10);
                ...
                function getHeaderHeight() {
                    return element[0].querySelector('[data-hook="wix-page-header"]').offsetHeight;
                }
            },
            templateUrl: "views/directives/wix-page-header.html",
            transclude: true,
            scope: {
                backgroundImageUrl: "@",
                gradientClassName: "@",
...
angular.module("wixStyle").directive("wixPageHeader", directives.wixPageHeaderDirective);

var directives;

(function(directives) {
    directives.wixPageContentDirective = function() {
        return {
            ...
            require: "^wixPage",
            link: function(scope, element, attrs, wixPageController) {
                var scrollContainerRef = element[0].querySelector('[data-hook="wix-page-content"]');
                if (scrollContainerRef) {
                    var scrollListenerRef_1 = scrollContainerRef.addEventListener("scroll", function() {
                        wixPageController.triggerHandler("scrollTop", scrollContainerRef.scrollTop);
                    });
                    scope.$on("$destroy", function() {
                        scrollContainerRef.removeEventListener("scroll", scrollListenerRef_1);
                    });
                }
                wixPageController.registerHandler("headerHeight", function(headerHeight) {
...
angular.module("wixStyle").directive("wixPageContent", directives.wixPageContentDirective);
